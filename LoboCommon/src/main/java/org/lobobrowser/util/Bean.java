...
public class Bean {
	...
	public PropertyDescriptor[] getPropertyDescriptors() throws IntrospectionException {
		synchronized (this) {
			return (PropertyDescriptor[]) getPropertyDescriptorsMap().values().toArray(new PropertyDescriptor[0]);
		}
	}
	...
	private void populateDescriptors(Map map, Class clazz) throws IntrospectionException {
		...
		if (clazz.isInterface()) {
			...
			for (final Type interface1 : interfaces) {
				populateDescriptors(map, (Class) interface1);
			}
		}
	}
	...
	public void setPropertyForFQN(Object receiver, String fullyQualifiedPropertyName, Object value) throws Exception {
		...
		if (idx == -1) {
			final PropertyDescriptor pd = getPropertyDescriptor(fullyQualifiedPropertyName);
			if (pd == null) {
			...
			final PropertyDescriptor pinfo = getPropertyDescriptor(prefix);
			if (pinfo == null) {
			...
			setPropertyForFQN(newReceiver, nameRest, value);
		}
	}
