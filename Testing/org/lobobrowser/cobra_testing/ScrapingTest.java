...
public class ScrapingTest {
    ...
    /** The Constant logger. */
    private static final Logger logger = Logger.getLogger(ScrapingTest.class
            .getName());

    public static void main(String[] args) throws Exception {
        UserAgentContext uacontext = new LocalUserAgentContext();
        LocalHtmlRendererContext rcontext = new LocalHtmlRendererContext(
                uacontext);
        ...
        if (startingDoc == null)
            throw new IllegalStateException(
                    "No document available for startup location.");
        XPath xpath = XPathFactory.newInstance().newXPath();
        NodeList nodeList = (NodeList) xpath.evaluate(SEARCH_FORM_XPATH,
                startingDoc, XPathConstants.NODESET);
        if (nodeList.getLength() == 0)
            throw new IllegalStateException(
                    "Expected search form not found in the page.");
        HTMLFormElementImpl form = (HTMLFormElementImpl) nodeList.item(0);

        // We now look for the text field where
        // the search phrase goes.
        HTMLInputElementImpl textInput = (HTMLInputElementImpl) startingDoc
                .getElementById(TEXT_FIELD_ID);
        if (textInput == null)
            throw new IllegalStateException("Did not find a text field named '"
                    + TEXT_FIELD_ID + "'.");
        ...
        FormInput submitInput1 = new FormInput(
                "icePage$SearchBoxTop$qkwsubmit.x", "1");
        FormInput submitInput2 = new FormInput(
                "icePage$SearchBoxTop$qkwsubmit.y", "1");
        ...
        if (searchResultsDoc == null)
            throw new IllegalStateException(
                    "No document available for search results page.");

        // Finally, we print out the search results.
        NodeList resultList = (NodeList) xpath.evaluate(
                "//a[@class='resultsLink']", searchResultsDoc,
    ...
    private static class LocalUserAgentContext extends SimpleUserAgentContext {
        ...
        /*
 * (non-Javadoc)
 * @see org.lobobrowser.html.test.SimpleUserAgentContext#isScriptingEnabled()
 */
        @Override
        ...
        /*
 * (non-Javadoc)
 * @see org.lobobrowser.html.test.SimpleUserAgentContext#isExternalCSSEnabled()
 */
        @Override
 ...
 */
    ...
    private static class LocalHtmlRendererContext extends
        ...
        /*
 * (non-Javadoc)
 * @see org.lobobrowser.html.AbstractHtmlRendererContext#isImageLoadingEnabled()
 */
        @Override
        ...
        /*
 * (non-Javadoc)
 * @see org.lobobrowser.html.AbstractHtmlRendererContext#getUserAgentContext()
 */
        @Override
            ...
            try {
                ...
                try {
                    ...
                    DocumentBuilderImpl builder = new DocumentBuilderImpl(
                            this.uacontext, this);
                    String charset = org.lobobrowser.util.Urls
