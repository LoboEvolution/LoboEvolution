 ...
 * the node's <code>name</code> or <code>id</code> attributes. Collections in
...
public interface HTMLOptionsCollection {
	 ...
	 * @return The <code>Node</code> at the corresponding position upon success. A
	 *         value of <code>null</code> is returned if the index is out of range.
	 */
	Node item(int index);

	/**
	 * This method retrieves a <code>Node</code> using a name. It first searches for
	 * a <code>Node</code> with a matching <code>id</code> attribute. If it doesn't
	 * find one, it then searches for a <code>Node</code> with a matching
	 * <code>name</code> attribute, but only on those elements that are allowed a
	 * name attribute. This method is case insensitive in HTML documents and case
