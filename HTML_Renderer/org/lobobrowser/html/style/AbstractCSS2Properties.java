...
public abstract class AbstractCSS2Properties extends AbstractScriptableDelegate implements CSS2Properties {
	...
	public void addStyleDeclaration(CSSStyleDeclaration styleDeclaration) {
		synchronized (this) {
			...
			for (int i = 0; i < length; i++) {
				...
				String priority = styleDeclaration
	 ...
	 *
	 * @param properties the new local style properties
	 */
	...
	protected final void setPropertyValueProcessed(String lowerCaseName,
		...
		SubPropertySetter setter = (SubPropertySetter) SUB_SETTERS
				.get(lowerCaseName);
		if (setter != null) {
	 ...
	 *
	 * @param value the new overlay color
	 */
	...
	public void setBorderColor(String borderColor) throws DOMException {
		this.checkSetProperty();
		new FourCornersSetter(BORDER_COLOR, "border-", "-color").changeValue(
	...
	public void setBorderStyle(String borderStyle) throws DOMException {
		this.checkSetProperty();
		new FourCornersSetter(BORDER_STYLE, "border-", "-style").changeValue(
	...
	public void setBorderWidth(String borderWidth) throws DOMException {
		this.checkSetProperty();
		new FourCornersSetter(BORDER_WIDTH, "border-", "-width").changeValue(
	...
	public void setPadding(String padding) throws DOMException {
		this.checkSetProperty();
		new FourCornersSetter(PADDING, "padding-", "").changeValue(this,
				padding, null);
		this.context.informInvalid();
	}
	...
	private static interface SubPropertySetter {
		 ...
		 * @param important the important
		 */
		public void changeValue(AbstractCSS2Properties properties,
				String newValue, CSSStyleDeclaration declaration,
				boolean important);
	}
	...
	private static class BorderSetter1 implements SubPropertySetter {
		...
		/* (non-Javadoc)
		 * @see org.lobobrowser.html.style.AbstractCSS2Properties.SubPropertySetter#changeValue(org.lobobrowser.html.style.AbstractCSS2Properties, java.lang.String, org.w3c.dom.css.CSSStyleDeclaration, boolean)
		 */
		public void changeValue(AbstractCSS2Properties properties,
				String newValue, CSSStyleDeclaration declaration,
				boolean important) {
	...
	private static class BorderSetter2 implements SubPropertySetter {
		 ...
		 */
		public void changeValue(AbstractCSS2Properties properties,
				String value, CSSStyleDeclaration declaration) {
			...
			if (value != null && value.length() > 0) {
				...
				for (int i = 0; i < array.length; i++) {
					...
					if (HtmlValues.isBorderStyle(token)) {
						style = token;
					} else if (org.lobobrowser.util.gui.ColorFactory
							.getInstance().isColor(token)) {
						color = token;
					} else {
						width = token;
					}
				...
				if (color != null) {
					properties.setPropertyValueLCAlt(name + "-color", color,
							important);
				}
				if (width != null) {
					properties.setPropertyValueLCAlt(name + "-width", width,
							important);
				}
				if (style != null) {
					properties.setPropertyValueLCAlt(name + "-style", style,
							important);
				}
			}
		}
	...
	private static class FourCornersSetter implements SubPropertySetter {
		...
		public void changeValue(AbstractCSS2Properties properties,
				...
				boolean important) {
			properties
					.setPropertyValueLCAlt(this.property, newValue, important);
			if (newValue != null && newValue.length() > 0) {
				...
				if (size == 1) {
					...
					properties.setPropertyValueLCAlt(prefix + "top" + suffix,
							value, important);
					properties.setPropertyValueLCAlt(prefix + "right" + suffix,
							value, important);
					properties.setPropertyValueLCAlt(
							prefix + "bottom" + suffix, value, important);
					properties.setPropertyValueLCAlt(prefix + "left" + suffix,
							value, important);
				} else if (size >= 4) {
					String prefix = this.prefix;
					String suffix = this.suffix;
					...
					properties.setPropertyValueLCAlt(prefix + "right" + suffix,
							array[1], important);
					properties.setPropertyValueLCAlt(
							prefix + "bottom" + suffix, array[2], important);
					properties.setPropertyValueLCAlt(prefix + "left" + suffix,
							array[3], important);
				} else if (size == 2) {
					String prefix = this.prefix;
					...
					properties.setPropertyValueLCAlt(prefix + "right" + suffix,
							array[1], important);
					properties.setPropertyValueLCAlt(
							prefix + "bottom" + suffix, array[0], important);
					properties.setPropertyValueLCAlt(prefix + "left" + suffix,
							array[1], important);
				} else if (size == 3) {
					String prefix = this.prefix;
		...
		public void changeValue(AbstractCSS2Properties properties,
			...
			if (newValue == null || !newValue.toLowerCase().startsWith(start)) {
				...
				if (closingIdx == -1) {
					...
					String tentativeUri = HtmlValues
							.unquoteAndUnescape(quotedUri);
					if (baseHref == null) {
						finalValue = newValue;
					} else {
						try {
			...
			properties.setPropertyValueLCAlt(BACKGROUND_IMAGE, finalValue,
					important);
		}
	}
	...
	private static class BackgroundSetter implements SubPropertySetter {
		...
		public void changeValue(AbstractCSS2Properties properties,
			...
			if (newValue != null && newValue.length() > 0) {
				...
				if (color != null) {
					properties.setPropertyValueLCAlt(BACKGROUND_COLOR, color,
							important);
				}
				if (image != null) {
					properties.setPropertyValueProcessed(BACKGROUND_IMAGE,
							image, declaration, important);
				}
				if (backgroundRepeat != null) {
					properties.setPropertyValueLCAlt(BACKGROUND_REPEAT,
							backgroundRepeat, important);
				}
				if (position != null) {
					properties.setPropertyValueLCAlt(BACKGROUND_POSITION,
							position, important);
				}
			}
		}
			...
			if (newValue != null && newValue.length() > 0) {
				...
				for (i = 0; i < length; i++) {
					...
					if (HtmlValues.isFontStyle(token)) {
						properties.setPropertyValueLCAlt(FONT_STYLE, token,
								important);
						continue;
					}
					if (HtmlValues.isFontVariant(token)) {
						properties.setPropertyValueLCAlt(FONT_VARIANT, token,
								important);
						continue;
					}
					if (HtmlValues.isFontWeight(token)) {
						properties.setPropertyValueLCAlt(FONT_WEIGHT, token,
								important);
						continue;
					}
					// Otherwise exit loop
				...
				if (token != null) {
					...
					String fontSizeText = slashIdx == -1 ? token : token
							.substring(0, slashIdx);
					properties.setPropertyValueLCAlt(FONT_SIZE, fontSizeText,
							important);
					String lineHeightText = slashIdx == -1 ? null : token
							.substring(slashIdx + 1);
					if (lineHeightText != null) {
						properties.setPropertyValueLCAlt(LINE_HEIGHT,
								lineHeightText, important);
					}
					if (++i < length) {
