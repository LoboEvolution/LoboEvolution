...
public class StyleSheetAggregator {
		...
		if (rule instanceof CSSStyleRule) {
			...
			if (uacontext.isExternalCSSEnabled()) {
				...
				if (CSSUtilities.matchesMedia(importRule.getMedia(), uacontext)) {
					...
					ArrayList<String> listext = CSSUtilities.cssText(href, document,
							document.getBaseURI());
	...
	private final void addClassRule(String elemtl, String classtl,
		...
		Collection<StyleRuleInfo> rules = (Collection<StyleRuleInfo>) classMap
				.get(classtl);
		if (rules == null) {
	...
	private final void addIdRule(String elemtl, String idtl,
		...
		Collection<StyleRuleInfo> rules = (Collection<StyleRuleInfo>) idsMap
				.get(idtl);
		if (rules == null) {
			rules = new LinkedList<StyleRuleInfo>();
	...
	public final Collection<CSSStyleDeclaration> getActiveStyleDeclarations(
		...
		Collection elementRules = (Collection) this.rulesByElement
				.get(elementTL);
		...
		if (className != null) {
			...
			Map<?, ?> classMaps = (Map<?, ?>) this.classMapsByElement
					.get(elementTL);
			if (classMaps != null) {
