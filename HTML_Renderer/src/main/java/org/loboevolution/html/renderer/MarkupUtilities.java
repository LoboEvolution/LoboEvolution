...
import org.loboevolution.util.Diagnostics;
...
public class MarkupUtilities {
	...
	private static BoundableRenderable findRenderable(Renderable[] renderables, int x, int y, int firstIndex,
		...
		if (length == 1) {
			...
			if (r instanceof BoundableRenderable) {
				...
				BoundableRenderable rleft = findRenderable(renderables, x, y, firstIndex, middleIndex - firstIndex,
						vertical);
				if (rleft != null) {
					return rleft;
				}
	...
	private static Range findRenderables(Renderable[] renderables, Rectangle clipArea, int firstIndex, int length,
		...
		if (offset1 == -1 && offset2 == -1) {
			// if(logger.isEnabled(Level.INFO))logger.info("findRenderables():
			// Range not found for clipArea="
