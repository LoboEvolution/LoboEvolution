 ...
 */
    ...
    public boolean extractSelectionText(StringBuffer buffer,
        ...
        if (result) {
            ...
            if (br != null) {
                ...
                if ((size > 0)
                        && !(renderables.get(size - 1) instanceof RBlank)) {
                    buffer.append(" ");
                }
            }
        }
    ...
    private final void addElement(RElement relement) throws OverflowException {
        ...
        case RElement.VALIGN_BOTTOM:
            ...
            break;
        case RElement.VALIGN_MIDDLE:
            requiredHeight = Math.max(ph, (ph / 2)
                    + (boundsh - this.baseLineOffset));
            break;
        default:
            requiredHeight = ph;
            break;
    ...
    private void adjustHeight(int newHeight, int elementHeight, int valign) {
        ...
        for (Iterator<Renderable> i = renderables.iterator(); i.hasNext();) {
            ...
            if (r instanceof RWord) {
                ...
                this.setElementY(relement, relement.getHeight(),
                        relement.getVAlign());
            } else {
 ...
 */
    ...
    public RenderableSpot getLowestRenderableSpot(int x, int y) {
        Renderable[] rarray = this.renderables.toArray(Renderable.EMPTY_ARRAY);
        BoundableRenderable br = MarkupUtilities.findRenderable(rarray, x, y,
                false);
        if (br != null) {
