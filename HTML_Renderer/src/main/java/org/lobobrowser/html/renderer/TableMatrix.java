...
public class TableMatrix {
    ...
    public void reset(Insets insets, int availWidth, int availHeight) {
        ...
        String borderText = this.tableElement
        ...
        String cellSpacingText = this.tableElement
                .getAttribute(HtmlAttributeProperties.CELLSPACING);
        int cellSpacing = 1;
        ...
        int widthsOfExtras = insets.left + insets.right
                + ((numCols + 1) * cellSpacing);
        if (border > 0) {
        ...
        int heightsOfExtras = insets.top + insets.bottom
                + ((numRows + 1) * cellSpacing);
        if (border > 0) {
    ...
    private static HtmlLength getWidthLength(HTMLElementImpl element,
        ...
        try {
            ...
            if (widthText == null) {
                String widthAttr = element
                        .getAttribute(HtmlAttributeProperties.WIDTH);
                if (widthAttr == null) {
                    return null;
                }
            ...
            if (ac != null) {
                if (ac.getVirtualRow() == row) {
                    ...
                    if (colSpan > 1) {
                        ...
                        if (denominator > 0) {
                            newTentativeCellWidth = (actualSize * (vcRenderWidth - cellExtras))
                                    / denominator;
                        } else {
