...
public class CSSStyleSheetListImpl implements StyleSheetList {
	...
	public StyleSheet merge() {
		...
		while (it.hasNext()) {
			final CSSStyleSheetImpl cssStyleSheet = (CSSStyleSheetImpl) it
					.next();
			final CSSMediaRuleImpl cssMediaRule = new CSSMediaRuleImpl(merged,
					null, cssStyleSheet.getMedia());
			cssMediaRule.setRuleList((CSSRuleListImpl) cssStyleSheet
					.getCssRules());
