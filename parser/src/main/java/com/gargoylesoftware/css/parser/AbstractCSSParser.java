...
public abstract class AbstractCSSParser implements CSSParser {
	...
	protected Locator createLocator(final Token t) {
		return new Locator(getInputSource().getURI(), t == null ? 0 : t.beginLine, t == null ? 0 : t.beginColumn);
	}
	...
	protected CSSParseException toCSSParseException(final String key, final ParseException e) {
		...
		message.append(")");
		return new CSSParseException(message.toString(), getInputSource().getURI(), e.currentToken.next.beginLine,
	...
	protected CSSParseException toCSSParseException(final DOMException e) {
		...
		return new CSSParseException(MessageFormat.format(messagePattern, e.getMessage()), getInputSource().getURI(), 1,
				1);
	}
	...
	protected CSSParseException toCSSParseException(final String messageKey, final Object[] msgParams,
	...
	protected void handleImportStyle(final String uri, final MediaQueryList media, final String defaultNamespaceURI,
			final Locator locator) {
	...
	protected void handleProperty(final String name, final LexicalUnit value, final boolean important,
			final Locator locator) {
	...
	protected LexicalUnit hexcolorInternal(final LexicalUnit prev, final Token t) {
		...
		try {
			if (len == 3) {
				...
				throw new CSSParseException(MessageFormat.format(pattern, new Object[] { t }),
			...
			final String pattern = getParserMessage("invalidColor");
			throw new CSSParseException(MessageFormat.format(pattern, new Object[] { t }), getInputSource().getURI(),
	 ...
	 * <a href="http://www.w3.org/TR/CSS21/syndata.html#escaped-characters">CSS
	 * specification</a>.
	 *
	 * This could be done directly in the parser, but portions of the lexer would
	 * have to be moved to the parser, meaning that the grammar would no longer
	 * match the standard grammar specified by the W3C. This would make the parser
	 * and lexer much less maintainable.
	 */
