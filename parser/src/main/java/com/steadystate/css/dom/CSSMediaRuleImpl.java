 ...
 * @author rbri
 */
public class CSSMediaRuleImpl extends AbstractCSSRuleImpl implements
		CSSMediaRule {
	...
	public CSSMediaRuleImpl(final CSSStyleSheetImpl parentStyleSheet,
			final CSSRule parentRule, final MediaList media) {
		...
		if (cssRules_ != null) {
			for (int i = 0; i < cssRules_.getLength(); i++) {
				...
				if (cssRule instanceof AbstractCSSRuleImpl) {
					...
					((AbstractCSSRuleImpl) cssRule)
							.setParentStyleSheet(getParentStyleSheetImpl());
				}
			}
		}
