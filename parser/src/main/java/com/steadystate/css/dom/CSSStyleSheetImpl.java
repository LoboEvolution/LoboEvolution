...
public class CSSStyleSheetImpl implements CSSStyleSheet, CSSFormatable, Serializable {
	...
	public int insertRule(final String rule, final int index) throws DOMException {
		...
		try {
			...
			throw new DOMExceptionImpl(DOMException.INDEX_SIZE_ERR, DOMExceptionImpl.INDEX_OUT_OF_BOUNDS,
					e.getMessage());
			...
			if (cssRule.getType() == CSSRule.IMPORT_RULE) {
				...
				try {
					...
					final CSSStyleSheetImpl importedCSS = (CSSStyleSheetImpl) new CSSOMParser()
							.parseStyleSheet(new InputSource(importURI.toString()), null, importURI.toString());
					if (recursive) {
