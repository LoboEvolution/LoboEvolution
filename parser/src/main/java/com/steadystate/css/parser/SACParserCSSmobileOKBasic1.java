 ...
 * @author waldbaer
 * @author rbri
 * @version $Id: SACParserCSSmobileOKBasic1.jj,v 1.5 2010-05-19 13:26:51
 *          waldbaer Exp $
 */
@SuppressWarnings("all")
public class SACParserCSSmobileOKBasic1 extends AbstractSACParser implements
		Parser, SACParserCSSmobileOKBasic1Constants {
		...
		try {
			...
			if (nonImportRuleFoundBefore) {
				this.getErrorHandler()
						.error(this.toCSSParseException(
								"invalidImportRuleIgnored", e));
			} else {
				this.handleImportStyle(unescape(t.image, false), ml, null,
						locator);
			}
	...
	final public void mediaRule() throws ParseException {
		...
		try {
			...
			CSSParseException cpe = this.toCSSParseException(
					"invalidMediaRule", e);
			this.getErrorHandler().error(cpe);
			this.getErrorHandler().warning(
					this.createSkipWarning("ignoringRule", cpe));
			this.error_skipblock();
		} finally {
			if (start) {
	...
	final public void styleRule() throws ParseException {
		...
		try {
			...
			this.getErrorHandler().error(
					this.toCSSParseException("invalidStyleRule", e));
			this.error_skipblock();
		} finally {
			if (start) {
	...
	final public Selector selector() throws ParseException {
		...
		try {
			...
			if (pseudoElementSel != null) {
				sel = this.getSelectorFactory().createDescendantSelector(sel,
						pseudoElementSel);
			}
	...
	final public Selector simpleSelector(Selector sel, char comb)
		...
		try {
			...
			case ASTERISK:
				...
				break;
			case HASH:
				simpleSel = this.getSelectorFactory().createElementSelector(
						null, null);
				c = hash(c);
				switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
				case DOT:
			...
			if (c != null) {
				simpleSel = this.getSelectorFactory()
			...
			if (sel != null) {
				switch (comb) {
				case ' ':
					sel = this.getSelectorFactory().createDescendantSelector(
							sel, simpleSel);
					break;
				}
			} else {
	...
	final public Condition _class(Condition pred) throws ParseException {
		...
		try {
			...
			locator = this.getLocator();
			t = jj_consume_token(IDENT);
			Condition c = this.getConditionFactory().createClassCondition(null,
					t.image);
			...
			{
				if (true)
					return (pred == null) ? c : this.getConditionFactory()
	...
	final public SimpleSelector elementName() throws ParseException {
		...
		try {
			...
			case IDENT:
				t = jj_consume_token(IDENT);
				sel = this.getSelectorFactory().createElementSelector(null,
	...
	final public Condition hash(Condition pred) throws ParseException {
		...
		try {
			...
			Condition c = this.getConditionFactory().createIdCondition(
	...
	final public LexicalUnit term(LexicalUnit prev) throws ParseException {
		...
		case STRING:
			t = jj_consume_token(STRING);
			value = new LexicalUnitImpl(prev, LexicalUnit.SAC_STRING_VALUE,
					t.image);
			break;
		case IDENT:
			t = jj_consume_token(IDENT);
	...
	private static void jj_la1_init_0() {
		jj_la1_0 = new int[] { 0x18000002, 0x18000002, 0x60012278, 0x40012278,
				0x60012278, 0x18000002, 0x18000002, 0x2, 0x60012278, 0x2,
				0x5000000, 0x2, 0x8, 0x2, 0x2, 0x12278, 0x1000, 0x2, 0x12278,
				0x2, 0x12278, 0x12278, 0x2, 0x2, 0x2, 0x21000, 0xc0000, 0x2,
				0x2, 0x4000, 0x2, 0x8, 0x2, 0x1000, 0x2, 0x2, 0x180, 0x200,
				0x2000, 0x70, 0x2000, 0x70, 0x70, 0x12278, 0x10008, 0x70,
				0x180, 0x8, 0x4000, 0x2, 0x8, 0x2, 0x80000000, 0x2, 0x50e1208,
				0x21000, 0xc0000, 0x0, 0x5000208, 0x2, 0x2, };
	}

	private static void jj_la1_init_1() {
		jj_la1_1 = new int[] { 0x0, 0x0, 0x1, 0x1, 0x1, 0x0, 0x0, 0x0, 0x1,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1,
				0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x8ffe, 0x0, 0x0,
				0x7fe, 0x8ffe, 0x0, 0x0, };
	}
