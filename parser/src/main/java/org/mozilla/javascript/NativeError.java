...
final class NativeError extends IdScriptableObject {
	...
	/**
	 * Default stack limit is set to "Infinity", here represented as a negative
	 * int
	 */
	...
	protected void fillConstructorProperties(IdFunctionObject ctor) {
		...
		// This is running on the global "Error" object. Associate an object
		// there that can store
		// default stack trace, etc.
		// This prevents us from having to add two additional fields to every
		// Error object.
		...
		ctor.defineProperty("prepareStackTrace", protoProps, ProtoProps.GET_PREPARE_STACK, ProtoProps.SET_PREPARE_STACK,
				0);
	...
	public Object getStackDelegated(Scriptable target) {
		...
		// Determine whether to format the stack trace ourselves, or call the
		// user's code to do it
		Object value;
		if (prepare == null) {
	...
	private static void js_captureStackTrace(Context cx, Scriptable thisObj, Object[] args) {
		...
		// Create a new error that will have the correct prototype so we can
		// re-use "getStackTrace"
		NativeError err = (NativeError) cx.newObject(thisObj, "Error");
		// Wire it up so that it will have an actual exception with a stack
		// trace
	...
	/**
	 * We will attch this object to the constructor and use it solely to store
	 * the constructor properties that are "global." We can't make them static
	 * because there can be many contexts in the same JVM.
	 */
