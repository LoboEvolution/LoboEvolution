...
public final class IRFactory extends Parser {
	...
	private Node transformArrayComp(ArrayComprehension node) {
		...
		try {
			...
			Node init = new Node(Token.EXPR_VOID, createAssignment(Token.ASSIGN, createName(arrayName), newArray),
					lineno);
		...
		if (functionType == FunctionNode.FUNCTION_EXPRESSION) {
			...
			if (name != null && name.length() != 0 && fnNode.getSymbol(name.getIdentifier()) == null) {
				...
				Node setFn = new Node(Token.EXPR_VOID, new Node(Token.SETNAME,
						Node.newString(Token.BINDNAME, name.getIdentifier()), new Node(Token.THISFN)));
