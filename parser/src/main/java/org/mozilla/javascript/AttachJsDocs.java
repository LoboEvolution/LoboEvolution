/* -*- Mode: java; tab-width: ; indent-tabs-mode: nil; c-basic-offset:  -*-
 *
 ...
 * recognized by the parser in order to attach jsdocs. On the other hand, in
 * Closure-style JavaScript the placement of jsdocs is syntax-driven; there is
 * intuitively some grammar that dictates where in the AST each comment should
 * be attached. In most cases, a comment C should be attached to the topmost AST
 * node that follows C in the source program. For example, in this code:
 * some_jsdoc bar(baz); the comment gets attached to the call node, not to bar.
 * There are some exceptions, eg, some infix expressions such as +: some_jsdoc +
 * ; In this code, the comment gets attached to , not to +.
 * <p>
 * For legacy reasons, we distinguish between two operators with different
 * precedence rules. CAST: a jsdoc followed by a left paren JSDOC: a jsdoc not
 * followed by a left paren Some operators have higher precedence than JSDOC but
 * lower than CAST; see the method betweenJsdocAndCast. For example, in this
 * code some_jsdoc obj.exp the jsdoc is attached to GETPROP, and in this code
 * some_jsdoc (obj).exp the jsdoc is attached to obj.
 * <p>
 * Algorithmically, we want to make a single pass over the AST and attach all
 * comments, not start from the root and search for each comment. This is
 * possible b/c the parser hands us the list of comments sorted by their
...
public class AttachJsDocs {
	...
	/**
	 * Returns the leftmost right sibling of n that satisfies
	 * finishesAfterJsdoc, o/w null.
	 * 
	 * @param parapos
	 *            The absolute position of n's parent.
	 */
	...
	/**
	 * Tries to attach currentJsdoc to n or one of its descendants. If the jsdoc
	 * starts after n, we return null. If the jsdoc is attached, we return the
	 * node on which it was attached. Last, if it was contained in n but didn't
	 * get attached, we return some node close to where the comment would get
	 * attached.
	 *
	 * The node that gets returned from this function is where the search for
	 * attaching the next comment will start from.
	 *
	 * A comment never gets attached to a node that appears before it in the
	 * code.
	 *
	 * @param parapos
	 *            n's parent's absolute position; we calculate it manually b/c
	 *            it's slow to call getAbsolutePosition for every node.
	 */
	...
	// Returns true iff n binds tighter than non-cast jsdocs & looser than
	// casts.
	private boolean betweenJsdocAndCast(AstNode n) {
		int ntype = n.getType();
		return (ntype == Token.GETPROP || ntype == Token.GETELEM
				|| ntype == Token.CALL || n instanceof Assignment || ((ntype == Token.INC || ntype == Token.DEC) && ((UnaryExpression) n)
				.isPostfix()));
	}
	...
	private boolean attachToChildren(AstNode n, int napos) {
		...
		if (ntype == Token.SCRIPT || ntype == Token.HOOK
				|| ntype == Token.RETURN || n instanceof ExpressionStatement
