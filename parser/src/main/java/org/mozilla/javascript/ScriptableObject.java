...
public abstract class ScriptableObject implements Scriptable, Serializable, DebuggableObject, ConstProperties {
	...
	private static class RelinkedSlot extends Slot {
		...
		RelinkedSlot(Slot slot) {
			...
			// Make sure we always wrap the actual slot, not another relinked
			// one
	...
	public void put(int index, Scriptable start, Object value) {
		if (externalData != null) {
			if (index < externalData.getArrayLength()) {
				...
				throw new JavaScriptException(ScriptRuntime.newNativeError(Context.getCurrentContext(), this,
						TopLevel.NativeErrors.RangeError, new Object[] { "External array index out of bounds " }), null,
						0);
			}
			return;
		}
	...
	/**
	 * Attach the specified object to this object, and delegate all indexed
	 * property lookups to it. In other words, if the object has 3 elements,
	 * then an attempt to look up or modify "[0]", "[1]", or "[2]" will be
	 * delegated to this object. Additional indexed properties outside the range
	 * specified, and additional non-indexed properties, may still be added. The
	 * object specified must implement the ExternalArrayData interface.
	 *
	 * @param array
	 *            the List to use for delegated property access. Set this to
	 *            null to revert back to regular property access.
	 * @since 1.7.6
	 */
	...
	/**
	 * Return the array that was previously set by the call to
	...
	/**
	 * This is a function used by setExternalArrayData to dynamically get the
	 ...
	 * @since 1.6R2
	 */
	public static <T extends Scriptable> String defineClass(Scriptable scope, Class<T> clazz, boolean sealed,
			boolean mapInheritance) throws IllegalAccessException, InstantiationException, InvocationTargetException {
	...
	static <T extends Scriptable> BaseFunction buildClassCtor(Scriptable scope, Class<T> clazz, boolean sealed,
			boolean mapInheritance) throws IllegalAccessException, InstantiationException, InvocationTargetException {
				...
				if (!isData && !isAccessor) {
					...
					if (isFalse(current.get("writable", current))) {
						if (isTrue(getProperty(desc, "writable"))) {
							throw ScriptRuntime.typeError1("msg.change.writable.false.to.true.with.configurable.false",
									id);
						}
					...
					if (isDataDescriptor(current)) {
						throw ScriptRuntime.typeError1("msg.change.property.data.to.accessor.with.configurable.false",
								id);
					} else {
						throw ScriptRuntime.typeError1("msg.change.property.accessor.to.data.with.configurable.false",
								id);
					}
				}
			}
	...
	/**
	 * Implements SameValue as described in ES5 9.12, additionally checking if
	...
	private synchronized Slot createSlot(String name, int indexOrHash, int accessType) {
		...
		Slot newSlot = accessType == SLOT_MODIFY_GETTER_SETTER ? new GetterSlot(name, indexOrHash, 0)
	 ...
	 * ids first, in numeric order, folowed by string ids, in insertion order.
	 * Since this class already keeps string keys in insertion-time order, we
	 * treat all as equal. The "Arrays.sort" method will then not change their
	 * order, but simply move all the numeric properties to the front.
	 */
