...
public class Parser {
	...
	private void addStrictWarning(String messageId, String messageArg, int position, int length, int line,
	...
	private void addWarning(String messageId, String messageArg, int position, int length, int line, String lineSource,
			int lineOffset) {
		String message = lookupMessage(messageId, messageArg);
		if (compilerEnv.reportWarningAsError()) {
	...
	private void addError(String messageId, String messageArg, int position, int length, int line, String lineSource,
			int lineOffset) {
		++syntaxErrorCount;
		String message = lookupMessage(messageId, messageArg);
		if (errorCollector != null) {
	...
	private AstNode arrowFunction(AstNode params) throws IOException {
		int baseLineno = ts.lineno; // line number where source starts
		int functionSourceStart = params != null ? params.getPosition() : -1; // start
																				// of
																				// "function"
																				// kwd
		...
		try {
			...
			if (!destructuring.isEmpty()) {
				...
				// Add assignment helper for each destructuring parameter
				for (Map.Entry<String, Node> param : destructuring.entrySet()) {
					Node assign = createDestructuringAssignment(Token.VAR, param.getValue(),
		...
		try {
			...
			if (pn != null) {
				if (compilerEnv.isStrictMode() && !pn.hasSideEffects()) {
					...
					addStrictWarning(pn instanceof EmptyStatement ? "msg.extra.trailing.semi" : "msg.no.side.effects",
							"", beg, nodeEnd(pn) - beg);
				}
				return pn;
			}
	...
	private Loop forLoop() throws IOException {
		...
		try {
			...
			} else if (compilerEnv.getLanguageVersion() >= Context.VERSION_ES6 && matchToken(Token.NAME)
	...
	void defineSymbol(int declType, String name, boolean ignoreNotInBlock) {
		...
		if (symbol != null && (symDeclType == Token.CONST || declType == Token.CONST
					...
					symDeclType == Token.CONST ? "msg.const.redecl"
							: symDeclType == Token.LET ? "msg.let.redecl"
									: symDeclType == Token.VAR ? "msg.var.redecl"
											: symDeclType == Token.FUNCTION ? "msg.fn.redecl" : "msg.parm.redecl",
					name);
			return;
		}
		switch (declType) {
	...
	private void checkBadIncDec(UnaryExpression expr) {
		...
		int tt = op.getType();
		if (!(tt == Token.NAME || tt == Token.GETPROP || tt == Token.GETELEM || tt == Token.GET_REF
				|| tt == Token.CALL)) {
	...
	boolean destructuringArray(ArrayLiteral array, int variableType, String tempName, Node parent,
		...
		for (AstNode n : array.getElements()) {
			...
			if (n.getType() == Token.NAME) {
				...
				parent.addChildToBack(
						destructuringAssignmentHelper(variableType, n, rightElem, currentScriptOrFn.getNextTempName()));
			}
			index++;
			empty = false;
		}
	...
	boolean destructuringObject(ObjectLiteral node, int variableType, String tempName, Node parent,
			List<String> destructuringNames) {
		boolean empty = true;
		int setOp = variableType == Token.CONST ? Token.SETCONST : Token.SETNAME;
