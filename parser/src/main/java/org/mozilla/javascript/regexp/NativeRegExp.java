...
public class NativeRegExp extends IdScriptableObject implements Function {
	...
	private static final byte REOP_UCFLAT1i = 19; /*
													 ...
													 * REOP_UCFLAT1
													 */
	// private static final byte REOP_UCFLAT = 20; /* flat Unicode string; len
	// immediate counts chars */
	// private static final byte REOP_UCFLATi = 21; /* case-independent
	// REOP_UCFLAT */
	...
	private static final byte REOP_JUMP = 32; /*
												 ...
												 */
	// private static final byte REOP_DOTSTAR = 33; /* optimize .* to use a
	// single opcode */
	// private static final byte REOP_ANCHOR = 34; /* like .* but skips left
	// context to unanchored r.e. */
	// private static final byte REOP_EOLONLY = 35; /* $ not preceded by any
	// pattern */
	// private static final byte REOP_BACKREFi = 37; /* case-independent
	// REOP_BACKREF */
	// private static final byte REOP_LPARENNON = 40; /* non-capturing version
	// of REOP_LPAREN */
	...
	private Object execSub(Context cx, Scriptable scopeObj, Object[] args, int matchType) {
		...
		if (d < 0 || str.length() < d) {
			...
			if ((re.flags & JSREG_GLOB) != 0) {
				lastIndex = rval == null || rval == Undefined.instance ? 0d : (double) indexp[0];
			}
		}
		return rval;
	}
	...
	private static boolean parseDisjunction(CompilerState state) {
		...
		if (index != source.length && source[index] == '|') {
			...
			if (result.kid.op == REOP_FLAT && result.kid2.op == REOP_FLAT) {
				...
				/*
				 * ALTPREREQ, uch1, uch2, <next>, ..., JUMP, <end> ... JUMP,
				 * <end>
				 */
				state.progLength += 13;
			} else if (result.kid.op == REOP_CLASS && result.kid.index < 256 && result.kid2.op == REOP_FLAT
					&& (state.flags & JSREG_FOLD) == 0) {
				result.op = REOP_ALTPREREQ2;
				result.chr = result.kid2.chr;
				result.index = result.kid.index;
				/*
				 * ALTPREREQ2, uch1, uch2, <next>, ..., JUMP, <end> ... JUMP,
				 * <end>
				 */
				state.progLength += 13;
			} else if (result.kid.op == REOP_FLAT && result.kid2.op == REOP_CLASS && result.kid2.index < 256
					&& (state.flags & JSREG_FOLD) == 0) {
				result.op = REOP_ALTPREREQ2;
				result.chr = result.kid.chr;
	...
	private static boolean parseTerm(CompilerState state) {
		...
		case '\\':
			if (state.cp < state.cpend) {
				...
				case '0':
					/*
					 * We're deliberately violating the ECMA 5.1 specification
					 * and allow octal escapes to follow spidermonkey and
					 * general 'web reality':
					 * http://wiki.ecmascript.org/doku.php?id=harmony:
					 * regexp_match_web_reality
					 * http://wiki.ecmascript.org/doku.php?id=strawman:
					 * match_web_reality_spec
					 */
	...
	private static void pushProgState(REGlobalData gData, int min, int max, int cp, REBackTrackData backTrackLastToSave,
			int continuationOp, int continuationPc) {
		gData.stateStackTop = new REProgState(gData.stateStackTop, min, max, cp, backTrackLastToSave, continuationOp,
				continuationPc);
	}
	...
	private static void pushBackTrackState(REGlobalData gData, byte op, int pc) {
		REProgState state = gData.stateStackTop;
		gData.backTrackStackTop = new REBackTrackData(gData, op, pc, gData.cp, state.continuationOp,
				state.continuationPc);
	}

	private static void pushBackTrackState(REGlobalData gData, byte op, int pc, int cp, int continuationOp,
			int continuationPc) {
	...
	private static boolean classMatcher(REGlobalData gData, RECharSet charSet, char ch) {
		...
		int byteIndex = ch >> 3;
		return (charSet.length == 0 || ch >= charSet.length || (charSet.bits[byteIndex] & 1 << (ch & 0x7)) == 0)
				^ charSet.sense;
	}
	...
	/*
	 * Apply the current op against the given input to see if it's going to
	 * match or fail. Return false if we don't get a match, true if we do and
	 * update the state of the input and pc if the update flag is true.
	 */
	private static int simpleMatch(REGlobalData gData, String input, int op, byte[] program, int pc, int end,
			boolean updatecp) {
	...
	private static boolean executeREBytecode(REGlobalData gData, String input, int end) {
		...
		for (;;) {
			...
			if (reopIsSimple(op)) {
				...
				case REOP_ASSERT: {
					int nextpc = pc + getIndex(program,
							pc); /* start of term after ASSERT */
				...
				case REOP_REPEAT: {
					...
					do {
						...
						continuationOp = REOP_REPEAT;
						continuationPc = pc;
						pushProgState(gData, new_min, new_max, startcp, null, state.continuationOp,
								state.continuationPc);
						if (new_min == 0) {
							pushBackTrackState(gData, REOP_REPEAT, pc, startcp, state.continuationOp,
									state.continuationPc);
				...
				case REOP_MINIMALREPEAT: {
					...
					if (!result) {
						...
						if (state.max == -1 || state.max > 0) {
							pushProgState(gData, state.min, state.max, gData.cp, null, state.continuationOp,
									state.continuationPc);
							continuationOp = REOP_MINIMALREPEAT;
							continuationPc = pc;
							int parenCount = getIndex(program, pc);
						...
						pushProgState(gData, new_min, new_max, gData.cp, null, state.continuationOp,
								state.continuationPc);
						if (new_min != 0) {
							continuationOp = REOP_MINIMALREPEAT;
							continuationPc = pc;
	...
	private static boolean matchRegExp(REGlobalData gData, RECompiled re, String input, int start, int end,
			boolean multiline) {
		if (re.parenCount != 0) {
...
class REProgState {
	REProgState(REProgState previous, int min, int max, int index, REBackTrackData backTrack, int continuationOp,
			int continuationPc) {
