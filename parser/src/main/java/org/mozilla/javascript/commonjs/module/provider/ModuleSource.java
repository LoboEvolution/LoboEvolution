 ...
 * <h1>Cache validators</h1> Validators are used by caches subclassed from
 ...
 * also allowed to carry expiration information (i.e. in case of HTTP expiration
 * header, or if a default expiration is used by the source provider to avoid
 * too frequent lookup of the resource), and to short-circuit the validation in
 * case the validator indicates the cached representation has not yet expired.
 * All these are plainly recommendations; the validators are considered opaque
 * and should only make sure to implement {@link Object#equals(Object)} as
 * caches themselves can rely on it to compare them semantically. Also, it is
 * advisable to have them be serializable.
 * 
