...
public class Require extends BaseFunction {
	 ...
	 */
	public Require(Context cx, Scriptable nativeScope, ModuleScriptProvider moduleScriptProvider, Script preExec,
		...
		if (id.startsWith("./") || id.startsWith("../")) {
			if (!(thisObj instanceof ModuleScope)) {
				throw ScriptRuntime.throwError(cx, scope,
						"Can't resolve relative module ID \"" + id + "\" when require() is used outside of a module");
			}
	...
	private Scriptable executeModuleScript(Context cx, String id, Scriptable exports, ModuleScript moduleScript,
			boolean isMain) {
		...
		return ScriptRuntime.toObject(cx, nativeScope, ScriptableObject.getProperty(moduleObject, "exports"));
	}
	...
	private ModuleScript getModule(Context cx, String id, URI uri, URI base) {
		try {
			final ModuleScript moduleScript = moduleScriptProvider.getModuleScript(cx, id, uri, base, paths);
			if (moduleScript == null) {
