...
public class JsonParser {
	...
	private String readString() throws ParseException {
		...
		while (pos < length) {
			...
			case 'u':
				...
				int code = fromHex(src.charAt(pos + 0)) << 12 | fromHex(src.charAt(pos + 1)) << 8
						| fromHex(src.charAt(pos + 2)) << 4 | fromHex(src.charAt(pos + 3));
				if (code < 0) {
	...
	private Boolean readFalse() throws ParseException {
		if (length - pos < 4 || src.charAt(pos) != 'a' || src.charAt(pos + 1) != 'l' || src.charAt(pos + 2) != 's'
				|| src.charAt(pos + 3) != 'e') {
