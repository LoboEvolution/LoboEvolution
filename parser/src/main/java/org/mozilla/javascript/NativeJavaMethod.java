...
public class NativeJavaMethod extends BaseFunction {
	...
	public String toString() {
		...
		for (MemberBox method2 : methods) {
			...
			sb.append(JavaMembers.liveConnectSignature(method2.argTypes));
			sb.append('\n');
		}
		return sb.toString();
	}
	...
	public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
		...
		if (meth.vararg) {
			...
			if (args.length == argTypes.length
					&& (args[args.length - 1] == null || args[args.length - 1] instanceof NativeArray
		...
		if (meth.isStatic()) {
			...
			for (;;) {
				if (o == null) {
					throw Context.reportRuntimeError3("msg.nonjava.method", getFunctionName(),
		...
		if (debug) {
			...
			logger.log(Level.ERROR, " ----- Wrapped as " + wrapped + " class = " + actualType);
		}
	...
	static int findFunction(Context cx, MemberBox[] methodsOrCtors, Object[] args) {
		...
		search: for (int i = 0; i < methodsOrCtors.length; i++) {
			...
			if (firstBestFit < 0) {
				...
				for (int j = -1; j != extraBestFitsCount; ++j) {
					...
					if (cx.hasFeature(Context.FEATURE_ENHANCED_JAVA_ACCESS) && (bestFit.member().getModifiers()
							& Modifier.PUBLIC) != (member.member().getModifiers() & Modifier.PUBLIC)) {
						...
						int preference = preferSignature(args, argTypes, member.vararg, bestFit.argTypes,
								bestFit.vararg);
		...
		if (methodsOrCtors[0].isCtor()) {
			throw Context.reportRuntimeError3("msg.constructor.ambiguous", memberName, scriptSignature(args),
					buf.toString());
		} else {
			throw Context.reportRuntimeError4("msg.method.ambiguous", memberClass, memberName, scriptSignature(args),
					buf.toString());
		}
	}
	 ...
	 */
	private static int preferSignature(Object[] args, Class<?>[] sig1, boolean vararg1, Class<?>[] sig2,
			boolean vararg2) {
