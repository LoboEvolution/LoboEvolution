 ...
 * @author Mike McCabe
 *
 *         Significant parts of this code are adapted from the venerable
 *         jsdate.cpp (also Mozilla):
...
final class NativeDate extends IdScriptableObject {
	...
	private static int YearFromTime(double t) {
		 ...
		 * computed using the average number of ms per year, it will usually be
		 * wrong for dates within several hours of a year transition.
		 */
		if (t2 > t) {
			y--;
		} else {
	...
	private static double parseISOString(String s) {
		...
		if (len != 0) {
			...
			if (c == '+' || c == '-') {
				...
				// time-only forms no longer in spec, but follow spidermonkey
				// here
				i += 1;
				state = HOUR;
			}
		...
		loop: while (state != ERROR) {
			...
			case TZHOUR:
				...
				// https://bugzilla.mozilla.org/show_bug.cgi?id=682754
				if (c != ':') {
		...
		syntax: {
			...
			if (tzhour == -1) {
				...
				// that local timezone was meant to be used. Stick with spec for
				// now.
	...
	private static double makeTime(double date, Object[] args, int methodId) {
		if (args.length == 0) {
			 ...
			 * arguments are set to undefined. Seems like all the
			 * Date.setWhatever functions in ECMA are only varargs beyond the
			 * first argument; this should be set to undefined if it's not
			 * given. This means that "d = new Date(); d.setMilliseconds()"
			 * returns NaN. Blech.
			 */
			return ScriptRuntime.NaN;
		}
