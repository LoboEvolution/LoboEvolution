...
/**
 * This class is the abstract parent for all of the various typed arrays. Each
 * one shows a view of a specific NativeArrayBuffer, and modifications here will
 * affect the rest.
 */
...
public abstract class NativeTypedArrayView<T> extends NativeArrayBufferView
	...
	/**
	 * Return the number of bytes represented by each element in the array. This
	 * can be useful when wishing to manipulate the byte array directly from
	 * Java.
	 */
	...
	private NativeArrayBuffer makeArrayBuffer(Context cx, Scriptable scope, int length) {
		return (NativeArrayBuffer) cx.newObject(scope, NativeArrayBuffer.CLASS_NAME, new Object[] { length });
	}
	...
	private void setRange(NativeTypedArrayView v, int off) {
		...
		if (v.arrayBuffer == arrayBuffer) {
			// Copy to temporary space first, as per spec, to avoid messing up
			// overlapping copies
	...
	private static final int Id_constructor = 1, Id_get = 2, Id_set = 3, Id_subarray = 4, Id_iterator = 5;

	protected static final int MAX_PROTOTYPE_ID = Id_iterator;
	...
	/*
	 * These must not conflict with ids in the parent since we delegate there
	 * for property dispatching.
	 */
	private static final int Id_length = 10, Id_BYTES_PER_ELEMENT = 11, MAX_INSTANCE_ID = Id_BYTES_PER_ELEMENT;
