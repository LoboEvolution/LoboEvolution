...
public class ClassFileWriter {
	...
	public static final short ACC_PUBLIC = 0x0001, ACC_PRIVATE = 0x0002, ACC_PROTECTED = 0x0004, ACC_STATIC = 0x0008,
			ACC_FINAL = 0x0010, ACC_SUPER = 0x0020, ACC_SYNCHRONIZED = 0x0020, ACC_VOLATILE = 0x0040,
	...
	final class StackMapTable {
		...
		void generate() {
			...
			if (DEBUGSTACKMAP) {
				...
				for (int i = 0; i < superBlocks.length && superBlocks[i] != null; i++) {
					System.out.println(
							"sb " + i + ": [" + superBlocks[i].getStart() + ", " + superBlocks[i].getEnd() + ")");
				}
			}
		...
		private void killSuperBlock(SuperBlock sb) {
			...
			for (int i = 0; i < itsExceptionTableTop; i++) {
				...
				if (sb.getStart() > eteStart && sb.getStart() < eteEnd
						|| eteStart > sb.getStart() && eteStart < sb.getEnd() && handlerSB.isInitialized()) {
		...
		private void executeBlock(SuperBlock work) {
			...
			for (int bci = work.getStart(); bci < work.getEnd(); bci += next) {
				...
				if (isBranch(bc)) {
					...
					if (DEBUGSTACKMAP) {
						System.out.println("sb " + work.getIndex() + " points to sb " + targetSB.getIndex()
								+ " (offset " + bci + " -> " + targetSB.getStart() + ")");
		...
		private void executeALoad(int localIndex) {
			...
			if (tag == TypeInfo.OBJECT_TAG || tag == TypeInfo.UNINITIALIZED_THIS
					|| tag == TypeInfo.UNINITIALIZED_VAR_TAG || tag == TypeInfo.NULL) {
				push(type);
			} else {
...
final class ConstantPool {
	...
	static final byte CONSTANT_Class = 7, CONSTANT_Fieldref = 9, CONSTANT_Methodref = 10,
...
final class TypeInfo {
	...
	static int merge(int current, int incoming, ConstantPool pool) {
		...
		throw new IllegalArgumentException(
				"bad merge attempt between " + toString(current, pool) + " and " + toString(incoming, pool));
	}
